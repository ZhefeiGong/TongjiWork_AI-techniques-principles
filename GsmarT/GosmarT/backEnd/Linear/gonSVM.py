...
class gonSVM:
    ...
    def svmTrain(self, trainDataIn, trainLabels, learningRate=1e-3, delta=1.0, alpha=1.0, epoch=2000, batchSize=200):
        ...
        # 权值初始化
        # if self.weight == None: --> 此时无论如何均进行训练
        self.weight = 0.001 * np.random.randn(featureNum, classesNum)         # 生成新的3073*10的二维数组
        # 开始训练
        midNum = int(epoch/400)                                               # 间隔
        ...
        for i in range(epoch):                                                # 显示进度条
            ...
            # 展示迭代情况
            if (i+1) % midNum == 0:                                                      # 每迭代100次 显示迭代情况
                lossHistory.append(loss)                                                 # 记录结果
                xHistory.append(i+1)                                                     # 记录结果
        return lossHistory, xHistory                                                     # 返回迭代过程损失函数值变化情况 用于可视化
...
# 进行训练
def linearTrain(svm,learningRate=1e-7, alpha=5e3, epoch=3000):
    trainData, trainLabels, testData, testLabels, verifyData, verifyLabels = Linear.tools.datasetGet()   # 数据集
    train_data = Linear.tools.preProcessing(trainData)                                                   # 数据预处理
    test_data = Linear.tools.preProcessing(testData)                                                     # 数据预处理
    lossHistory, xHistory = svm.svmTrain(train_data, trainLabels, learningRate, 1.0, alpha, epoch)       # 记录损失情况
    res = svm.svmPredict(test_data)                                                                      # 记录预测结果
    return (np.mean(res == testLabels) * 100), xHistory, lossHistory

# 进行预测
def linearPredict(svm):
    image = cv2.imread('./Linear/image/ori.jpg')
    if len(image) >= 32 and len(image[0]) >= 32:
        res = cv2.resize(image, (32, 32), interpolation=cv2.INTER_CUBIC)  # 裁剪cifar10中的32*32*3
        test = Linear.tools.dim3Todim2(res)
        return svm.svmPredict(test)
    else:
        return -1
...
if __name__ == '__main__' :
    ...
    # 可视化
    Linear.tools.graphShow(np.linspace(1, 3000, 3000), lossHistory, 'times', 'loss', 'LossValueGraph')  # 可视化结果
